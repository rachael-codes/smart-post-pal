...
serve(async (req) => {
  ...
  try {
    ...
    const pricing = {
      ...
      enterprise: { amount: 4900, name: "Enterprise Plan" }
    };
      ...
      line_items: [
        {
          price_data: {
            currency: "usd",
            product_data: { 
              name: `SmartPost AI - ${selectedPlan.name}`,
              description: `${plan === 'pro' ? 'Professional' : 'Enterprise'} social media scheduling with AI features`
            },
